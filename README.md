...
Версия 0.4.0: [этап 4A — локальный диспетчер Codex](docs/stage-4a-dispatcher.md). Автозапуск по явному opt-in, настройка модели, управление сессиями, журнал восстановления и доставка отчёта. Диспетчер запускается отдельным процессом; MCP-сервер по-прежнему не запускает исполнителей. Для 4B зафиксирована [гипотеза возобновления planner-разговора через heartbeat](docs/stage-4b-scheduler-resume-research.md); end-to-end проверка ещё не выполнена.
